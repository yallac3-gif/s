...
send_telegram() {
    ...
    if [[ "$RESPONSE" == *"\"ok\":true"* ]]; then
        ...
        log_message "❌ Telegram Error: $RESPONSE"
    fi
}
...
if ! npm install --no-audit --no-fund; then
    ...
    send_telegram "❌ <b>Rotation Failed:</b> npm install failed."
    exit 1
fi

log_message "Building frontend assets..."
# Capture build output to a temporary file
BUILD_LOG="build_error.log"
if ! npm run build > "$BUILD_LOG" 2>&1; then
    log_message "Error: Build failed. Printing build logs below:"
    echo "---------------- BUILD ERROR LOG ----------------"
    cat "$BUILD_LOG"
    echo "-------------------------------------------------"
    send_telegram "❌ <b>Rotation Failed:</b> Build step failed. Check GitHub Action logs for details."
    exit 1
fi
rm "$BUILD_LOG"
